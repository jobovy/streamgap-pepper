...
def contour2D(h2,x,y,axis,with_imshow=False,with_contours=True,label=None,fabsflag=False,text=None,cmap=plt.cm.hot,with_kde=False,kde_bandwidth=0.01,alpha=1.):
	...
	if(with_imshow):
		c = axis.imshow(np.rot90(h2/np.max(h2)),cmap=cmap,extent=ex,aspect='auto',vmin=0.,vmax=1.,alpha=alpha)
	if(with_contours):
		c = axis.contour(.5*(x[1:]+x[:-1]),.5*(y[1:]+y[:-1]),h2.T,
	                 origin='lower',
...
def plot_2D_power(data1,data2,ax,axcorr=None):
	angpar=np.histogram2d(data1,data2,bins=400)
	FFT = np.fft.fft2(angpar[0])
	deltaX = (angpar[1][-1]-angpar[1][0])/len(angpar)
	deltaY = (angpar[2][-1]-angpar[2][0])/len(angpar)
	Xrange = np.fft.fftfreq(angpar[1].size, d=deltaX)
	Yrange = np.fft.fftfreq(angpar[2].size, d=deltaY)
	contour2D(FFT*np.conj(FFT),Xrange,Yrange,ax)
